        ...
        for path in self.pathPool:
            ...
            if 0<=pathCount<5:
                if pathFlag==0:# pathFlag为0，代表无课
                    self.classRoomDataJsonText[responseObject["classRoomName"]]["am12"][str(dayCount)].append(int(responseObject["classRoomNum"]))
            elif 5<=pathCount<10:
                if pathFlag==0:
                    self.classRoomDataJsonText[responseObject["classRoomName"]]["am34"][str(dayCount)].append(int(responseObject["classRoomNum"]))
            elif 10<=pathCount<15:
                if pathFlag==0:
                    self.classRoomDataJsonText[responseObject["classRoomName"]]["pm12"][str(dayCount)].append(int(responseObject["classRoomNum"]))
            elif 15<=pathCount<20:
                if pathFlag==0:
                    self.classRoomDataJsonText[responseObject["classRoomName"]]["pm34"][str(dayCount)].append(int(responseObject["classRoomNum"]))
